...
#ifdef USE_SOUND_GEN
#include "SSS_SoundLib/sss_soundLib.h"
#include "SSS_SoundLib/test_ui_snd1.h"
#endif
...
void flash_send_page (uint16_t pagenum) {
    ...
    // LOG_PRINT("pagenum:%u", (unsigned int)pagenum);
...
void flash_create_filemap (uint16_t file_id) {
    ...
    // LOG_PRINT ("page_addr_total: %u\r\n", (unsigned int)founded_pages_cnt);

    if (founded_pages_cnt == 0) {

        // LOG_PRINT ("send_status_233...\r\n");
        address_map_ptr = 0;
...
void execute_xproto_cmd() {
    ...
    // LOG_PRINT ("CRC Rx: 0x%02X | CRC Cal: 0x%02X\r\n", _header.crc8, cal_crc);
    ...
    if (_target.PackIdent == XP_FLASHPAGE_CONTWRITE_PACKET) {
        ...
        if (flash_hw->nonx_area_begin > 0 && flash_hw->nonx_area_begin < flash_hw->chip_size && _current_page_writecont >= flash_hw->nonx_area_begin) {
            ...
            // LOG_PRINT ("XP_FLASHPAGE_CONTWRITE_PACKET 2\r\n");
...
int main (void) {
    ...
    LOG_PRINT ("SystemClk:%u\r\n", (unsigned int)SystemCoreClock);
    LOG_PRINT ("ChipID:%08x\r\n", (unsigned int)DBGMCU_GetCHIPID());
    //-------------------------------------------------------------------------
    Delay_Ms (500);
    //-------------------------------------------------------------------------
    /* static const PinDefine_t TEST_PORT = {// PC1
                                             .rcc = RCC_APB2Periph_GPIOC,
                                             .port = GPIOC,
                                             .pin = GPIO_Pin_1};

    fgpio_openPin (&TEST_PORT, GPIO_Mode_Out_PP);
    fgpio_setPin(&TEST_PORT, 1);
    Delay_Ms(1000); */
    //-------------------------------------------------------------------------
#ifdef USE_FLASH_W25Q
    ...
    if (flash_hw->nonx_area_begin > 0) {
        ...
        LOG_PRINT ("Cont Area Begin: %u\r\n", (unsigned int)flash_hw->nonx_area_begin);
    }
    // ------------------------------------------------------------------------
    //fgpio_setPin (&TEST_PORT, 0);
#ifdef USE_TFT_ST7789
    ...
    //-------------------------------------------------------------------------
#endif
    LOG_PRINT ("Running (v_CH32X033 Programmer)...\r\n");
    //-------------------------------------------------------------------------
#ifdef USE_OLED_SSD1306_TXT_I2C
    OLED_TXT_Promo();
#endif
