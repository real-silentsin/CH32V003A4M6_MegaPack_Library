...
#include <string.h>
// -----------------------------------------------------------------------------
// Звуковой ШИМ-сигнал выводится на пин PC0 (TIM1, Канал 3)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// Настройка Таймера 1 для генерации несущей частоты ШИМ (8 бит, ~187 кГц)
void Init_PWM_Timer1 (void) {
    ...
    RCC_APB2PeriphClockCmd (SND_OUT_PORT.rcc | RCC_APB2Periph_AFIO | RCC_APB2Periph_TIM1, ENABLE);

    SOUND_TIM1_REMAP();

    // 2. Настраиваем физический пин PС0 на вывод ШИМ
    ...
    // 4. Конфигурация Канала 1 (TIM1_CH3 на PC0) в режим ШИМ
    TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
    TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
    TIM_OCInitStructure.TIM_Pulse = 128;       // Середина шкалы (тишина)
    TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;
    TIM_OC3Init (TIM1, &TIM_OCInitStructure);  // Строго OC3
